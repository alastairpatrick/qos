...
static inline void qos_init_dnode(struct qos_dnode_t* node) {
  node->next = node;
  node->prev = node;
}

static inline void qos_init_dlist(struct qos_dlist_t* list) {
  qos_init_dnode(&list->sentinel);
}

static inline bool qos_is_dlist_empty(qos_dlist_t* list) {
  return list->sentinel.next == &list->sentinel;
}

static inline void qos_splice_dnode(qos_dnode_t* dest, qos_dnode_t* source) {
...
static inline void qos_remove_dnode(struct qos_dnode_t* node) {
